...
from src.morphology_paradigm_v7 import report


def test_v7_runtime_score_shape() -> None:
    value = report()
    combined = value["combined"]
    master = value["master"]

    assert combined["positive_row_count"] == 8
    assert combined["positive_unique_surface_count"] == 6
    assert master["positive_unique_surface_count"] == 6
    assert combined["unknown_count"] == 8
    assert master["unknown_count"] == 8


def test_v7_unknown_safety_is_preserved() -> None:
    value = report()
...
def test_v7_measurement_does_not_change_runtime_or_learn_answers() -> None:
    value = report()
    ...
    assert integrity["runtime_rule_learning_from_v7_allowed"] is False
