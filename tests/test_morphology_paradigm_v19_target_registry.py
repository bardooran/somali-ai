...
def test_v19_targets_are_registered_before_answer_lookup() -> None:
    record = _registry()
    ...
    assert record["target_person"] == "3sg_feminine"
    ...
    assert record["selection_commit_base"] == (
        "f278834fd7102f9a6d845a7f8953357269b60c58"
    )
...
def test_v19_targets_are_class_known_and_not_target_specific_profiles() -> None:
...
def test_v19_selection_pins_pre_answer_six_cell_class_past_scope() -> None:
    ...
    assert state["generic_3sg_feminine_past_activation_exists"] is False
    assert state["target_specific_profiles_exist"] is False
    assert state["v19_answers_used_as_runtime_evidence"] is False
    assert state["one_surface_can_have_multiple_person_analyses"] is True
    assert state["documented_2sg_3sg_feminine_syncretism_predates_v19"] is True


def test_v19_registry_contains_no_answer_surface_fields_or_answers() -> None:
    record = _registry()
    ...
    assert record["pre_answer_state"]["v19_answers_used_as_runtime_evidence"] is False
    for lemma in TARGETS:
        assert lemma in raw
