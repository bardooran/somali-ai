...
def test_v16_targets_are_registered_before_answer_lookup() -> None:
    record = _registry()
    ...
    assert record["target_person"] == "1pl"
...
def test_v16_targets_are_class_known_and_not_target_specific_profiles() -> None:
...
def test_v16_selection_pins_pre_answer_three_cell_class_past_scope() -> None:
    state = _registry()["pre_answer_state"]

    # Historical snapshot only: future post-freeze activation may add 1pl.
    assert state["authorized_class_past_persons"] == ["2sg", "2pl", "3pl"]
    assert state["generic_1pl_past_activation_exists"] is False
    assert state["target_specific_profiles_exist"] is False
    assert state["v16_answers_used_as_runtime_evidence"] is False


def test_v16_registry_contains_no_answer_surface_fields_or_answers() -> None:
    record = _registry()
    ...
    assert record["pre_answer_state"]["v16_answers_used_as_runtime_evidence"] is False
    for lemma in TARGETS:
        assert lemma in raw
