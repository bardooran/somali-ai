...
from src.morphology_paradigm_v18 import report
from src.morphophonology_generator import eligible_conj2_profile_lemmas

BENCHMARK = Path("data/qa/morphology_paradigm_benchmark_v18.jsonl")
METADATA = Path("data/qa/morphology_paradigm_benchmark_v18.meta.json")
...
def test_v18_manifest_is_frozen_to_two_attested_3sg_m_rows_and_eight_unknowns() -> None:
    rows = _rows()
...
def test_v18_metadata_pins_manifest_and_pre_freeze_scope_historically() -> None:
    metadata = json.loads(METADATA.read_text(encoding="utf-8"))

    assert metadata["benchmark_version"] == "v18"
    assert metadata["manifest_git_blob_sha"] == _git_blob_sha(BENCHMARK)
    assert metadata["pre_answer_target_registry_merge_commit"] == (
        "d899f769f13374ab497d0c3a04c2259c0925cfe4"
    )
    assert metadata["selected_target_lemmas"] == ["aammusi", "abhi"]
    assert metadata["scored_target_lemmas"] == ["aammusi", "abhi"]
    ...
    assert state["generic_3sg_masculine_past_authorized"] is False
    assert state["generic_3sg_feminine_past_authorized"] is False
    assert state["existing_1sg_3sg_masculine_syncretism_documented"] is True
    assert state["existing_1sg_surface_generation_authorized"] is True


def test_v18_answer_evidence_is_lexicographic_and_runtime_isolation_is_explicit() -> None:
    metadata = json.loads(METADATA.read_text(encoding="utf-8"))
    ...
    assert all(item["attested_entry"].endswith("(-iyay, -isay)") for item in sources)
    assert all(
        item["evidence_type"]
    ...
    assert policy["runtime_rule_learning_from_v18_allowed"] is False
    ...
    assert policy[
        "surface_recognition_must_not_be_equated_with_3sg_masculine_person_resolution"
    ] is True
    assert policy[
...
def test_v18_registry_proves_targets_were_fixed_without_answers() -> None:
    registry = json.loads(REGISTRY.read_text(encoding="utf-8"))

    assert registry["status"] == "pre_answer_target_registry"
    assert registry["target_lemmas"] == ["aammusi", "abhi"]
...
def test_v18_targets_remain_outside_target_specific_profile_path() -> None:
    profiles = set(eligible_conj2_profile_lemmas())
    assert "aammusi" not in profiles
    assert "abhi" not in profiles


def test_v18_scorer_separates_recognition_person_resolution_and_syncretism() -> None:
    result = report()
    ...
    assert diagnostics["expected_persons"] == ["1sg", "3sg_m"]
    assert diagnostics["syncretic_surface_count"] == 2
    # 1SG is already a reviewed class-level past cell before v18 and shares the
    # same surface shape; future 3SG-m activation must add, not replace, it.
    ...
    assert result["interpretation"]["syncretism_with_1sg_is_expected"] is True
