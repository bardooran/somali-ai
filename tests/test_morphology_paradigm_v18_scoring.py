...
def test_v18_scorer_preserves_frozen_dimensions() -> None:
    result = report()
...
def test_v18_scorer_keeps_preselected_targets_separate_from_runtime_authority() -> None:
    result = report()
    preauthorization = result["preauthorization"]

    assert preauthorization["target_selection_predates_answer_lookup"] is True
    assert preauthorization["generic_3sg_masculine_past_authorized_at_freeze"] is False
    assert preauthorization["scored_target_lemmas"] == ["aammusi", "abhi"]
    assert preauthorization["unresolved_target_lemmas"] == []
    assert set(preauthorization["selected_targets"]) == {"aammusi", "abhi"}
...
def test_v18_measurement_preserves_unknown_safety_and_evaluation_boundary() -> None:
    result = report()
    ...
    assert integrity["runtime_rule_learning_from_v18_allowed"] is False
...
def test_v18_metadata_locks_untouched_historical_baseline() -> None:
    benchmark = report()["benchmark"]
    measured = benchmark["measured_result"]

    assert benchmark["measurement_status"] == "measured"
    assert measured["full_test_suite"] == "1195/1195 passed"
    assert measured["pull_request"] == 56
    assert measured["tested_head_commit"] == "32904a205ba8f789415e3b1e9acaa1e75227d1a2"
    assert measured["workflow_run_id"] == 33496886364
    assert measured["workflow_job_id"] == 99821019191
    ...
    assert measured["somali_ai_combined_3sg_m_person_matches"] == "0/2"
    assert measured["somali_ai_combined_deep_feature_rows"] == "0/2"
    assert measured["somali_ai_existing_1sg_analysis"] == "2/2"
    assert measured["somali_ai_1sg_3sg_m_syncretism_preserved"] == "0/2"
    ...
    assert "3sg_m person and deep-feature resolution remained 0/2" in benchmark["notes"]


def test_v18_scorer_exposes_syncretism_without_forcing_unique_person() -> None:
    ...
    assert set(diagnostics["observed_persons_by_surface"]) == {"aammusiyay", "abhiyay"}


def test_v18_scorer_does_not_claim_full_paradigm_or_global_win() -> None:
    interpretation = report()["interpretation"]

    assert interpretation["v18_tests_preselected_c2a_3sg_masculine_past"] is True
    assert interpretation["surface_recognition_is_not_person_resolution"] is True
    assert interpretation["syncretism_with_1sg_is_expected"] is True
