"""Score frozen analyzer-blind morphology challenge v3.

v3 was frozen before the master recognition bridge existed. The benchmark
therefore compares the conservative reviewed analyzer and the broader exact
master-store recognition layer without changing any test case.
"""
...
MANIFEST_PATH = Path("data/qa/morphology_challenge_v3.jsonl")
TARGET_TYPES = {"noun", "verb", "adjective", "numeral"}


@dataclass(frozen=True)
class ChallengeV3Score:
    system: str
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line in handle:
            ...
            if not isinstance(case, dict):
                raise ValueError("v3 benchmark row must be an object")
            case_id = str(case.get("id", ""))
            split = str(case.get("split", ""))
            expected = case.get("expected_analyses")
            if not case_id or case_id in seen:
                raise ValueError(f"invalid or duplicate v3 id: {case_id!r}")
            if case.get("benchmark_version") != "v3":
                raise ValueError(f"wrong benchmark version for {case_id}")
            if split not in {"challenge", "unknown"}:
                raise ValueError(f"invalid v3 split for {case_id}: {split!r}")
            ...
            if split == "unknown" and expected:
                raise ValueError(f"unknown v3 case has positive analyses: {case_id}")
            seen.add(case_id)
            cases.append(case)
    return tuple(cases)
...
def report(path: Path = MANIFEST_PATH) -> dict:
    ...
    master_confidence_counts = {tier: 0 for tier in ("trusted", "supported", "provisional")}
    for case in cases:
        if case["split"] != "challenge":
            continue
        tiers = confidence.get(str(case["surface"]).casefold(), set())
        for tier in master_confidence_counts:
            if tier in tiers:
                master_confidence_counts[tier] += 1
    return {
        "reviewed": {
