...
The affirmative-future layer follows the same evidence-first policy. Qaamuus supplies the exact `cuni + doonaa/doontaa/doonnaa/doontaan/doonaan` paradigm. Gothenburg explicitly gives Class-II `raadin doonaa`, and independent Radio Sweden running text supplies `raadin doonnaa` and `raadin doonaan`. The analyzer therefore has a separate exact future-stem candidate `raadin` for lemma `raadi`; it does not infer that other Class-II verbs ending in `-i` automatically have an `-in` future stem. The same surface `raadin` remains context-sensitive because it is separately attested in habitual and verbal-noun uses.
